...
function matchesCategory(product, category) {
  ...
  const aliases = getSlugAliases(normalizedCategory);
...
function SearchPageComponent() {
  ...
  const searchedProducts = useMemo(() => {
    return dbProducts.filter((item) => {
      ...
      const categoryName = normalizeText(getCategoryName(item));
      const submenuName = normalizeText(getSubmenuName(item));
      const categorySlug = normalizeText(getCategorySlug(item));
      const submenuSlug = normalizeText(getSubmenuSlug(item));
      ...
      const matchKeyword =
        ...
        submenuSlug.includes(keyword) ||
        description.includes(keyword);

      const matchCategory = matchesCategory(item, category);

      return matchKeyword && matchCategory;
    });
  }, [dbProducts, keyword, category]);
  ...
  }, [searchedProducts, filterByPrice]);
  ...
  return (
    <div>
      ...
      <div className="container category-page">
        <h1 className="category-page-title">Kết quả tìm kiếm</h1>
        <p className="category-page-desc">
          Từ khóa: <strong>{keyword || 'Tất cả'}</strong>
          <br />
          Danh mục: <strong>{category === 'all' ? 'Tất cả' : category}</strong>
        </p>
        ...
        {loadingDb ? (
          ...
          <p className="no-products-text">Không tìm thấy sản phẩm phù hợp.</p>
        ) : (
